...
<cite>
**本文档引用的文件**  
- [file_manager.py](file://utils/file_manager.py#L1-L431) - *已移除同步功能*
- [config.py](file://common/config.py#L1-L170) - *已移除同步相关配置*
- [hajimi_king.py](file://app/hajimi_king.py#L1-L512) - *已移除同步逻辑*
</cite>

## 更新摘要
**变更内容**  
- 移除了与同步功能相关的所有代码，包括 `wait_send_balancer` 和 `wait_send_gpt_load` 字段
- 更新了 `Checkpoint` 类结构，仅保留 `last_scan_time`、`scanned_shas` 和 `processed_queries`
- 移除了 `FileManager` 中与 `KEYS_SEND` 相关的文件名、属性和方法
- 相应地更新了配置文件 `config.py`，移除了同步相关的环境变量和日志输出
- 所有文档内容已同步反映这些变更，确保与最新代码一致
...
- `app/`：主程序入口，包含核心业务逻辑。
- `common/`：通用组件，如日志记录器和配置管理。
- `scripts/`：辅助脚本，用于测试或部署。
- `utils/`：工具类模块，包括文件管理、GitHub客户端和同步工具。

其中，`utils/file_manager.py` 是实现状态持久化和文件操作的核心模块，负责维护扫描进度、保存结果和管理检查点。

```mermaid
graph TD
A[file_manager.py] --> B[Checkpoint]
A --> C[FileManager]
D[config.py] --> A
E[hajimi_king.py] --> A
E --> D
C --> F[checkpoint.json]
C --> G[scanned_shas.txt]
C --> H[keys_valid_*.txt]
C --> I[key_429_*.txt]
```
...
`file_manager.py` 模块的核心职责是实现**断点续传**和**状态持久化**。通过将扫描状态保存到磁盘文件中，系统在重启后可以恢复上次的扫描进度，避免重复工作。该模块主要由两个类构成：

- `Checkpoint`：定义扫描状态的数据结构。
- `FileManager`：负责所有文件的读写操作，包括检查点加载、密钥保存、日志记录等。
...
**图示来源**  
- [file_manager.py](file://utils/file_manager.py#L12-L42)
...
`FileManager` 类负责所有与文件相关的操作，其核心功能包括：
...
**图示来源**  
- [file_manager.py](file://utils/file_manager.py#L173-L237)

**本节来源**  
- [file_manager.py](file://utils/file_manager.py#L50-L237)
...
在 `hajimi_king.py` 中，`should_skip_item()` 函数利用此时间戳判断仓库是否需要跳过：
...
这些状态最终通过 `file_manager.save_checkpoint(checkpoint)` 持久化到磁盘。

**本节来源**  
- [file_manager.py](file://utils/file_manager.py#L47)
- [hajimi_king.py](file://app/hajimi_king.py#L220-L250)

## 文件读写安全与异常处理

为确保数据完整性，`FileManager` 实现了多层安全保障。
...
**本节来源**  
- [file_manager.py](file://utils/file_manager.py#L238-L431)
...
`file_manager.py` 模块通过精心设计的 `Checkpoint` 类和 `FileManager` 类，实现了高效、可靠的状态管理机制。其核心优势包括：
