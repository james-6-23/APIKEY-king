...
交互式向导会依次询问 **安装目录 / 监听端口 / 管理员密码**（回车使用默认值）。端口被占用会自动提示重新输入；JWT 密钥每次安装随机生成。整个过程无需 clone 仓库。

> 要求：已安装 `docker` + `docker compose`（v2 插件）或 `docker-compose`（v1）。没有 docker 的参考 <https://get.docker.com>。
