...
function displayKeys(keys) {
    ...
    pageKeys.forEach((key, index) => {
        ...
        tr.innerHTML = `
            <td class="w-[40px] px-2 py-3 text-center">
            ...
            <td class="w-[100px] px-2 py-3 whitespace-nowrap overflow-hidden">
                <span class="inline-flex px-2 py-1 text-xs font-medium rounded ${getKeyTypeBadge(key.type)}">
                    ${key.type.toUpperCase()}
                </span>
            </td>
            <td class="w-[140px] px-2 py-3 font-mono text-xs truncate overflow-hidden" title="${escapeAttribute(key.key)}">${escapeHtml(key.key.substring(0, 16))}...</td>
            <td class="w-[50px] px-2 py-3 text-xs whitespace-nowrap overflow-hidden">${balanceDisplay}</td>
            <td class="w-[280px] px-2 py-3 text-xs truncate overflow-hidden">
            ...
            <td class="w-[150px] px-2 py-3 text-xs text-slate-600 whitespace-nowrap overflow-hidden">${key.found_at}</td>
            <td class="w-[60px] px-2 py-3 text-center whitespace-nowrap overflow-hidden">
                <button onclick="copyKey('${escapeAttribute(key.key)}')" class="px-2 py-1 text-xs bg-primary text-white rounded hover:bg-primary/90 transition-colors">
                    复制
                </button>
            </td>
        `;
